...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        for col1 in qi.domain():
            for col2 in qj.domain():
                if col1 != col2 and abs(i - j) != abs(col1 - col2):
                    satisfyingAssignments.append([col1, col2])
...
class NValuesConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], valsInRange)
        return x
